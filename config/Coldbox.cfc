	...
	function configure() {
		...
		logBox = {
			...
			appenders : { 
				...
				SystemLog : { class="coldbox.system.logging.appenders.RollingFileAppender", properties={ filePath=expandPath("logs"),autoExpand=false } }						  
			},

			categories : {
				'coldbox.system' : { levelmin='info', appenders='SystemLog' },
			}	
		};
